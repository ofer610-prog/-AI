...
export async function runWhatsappScan() {
  if (!BASE_URL || !TOKEN) {
    return { error: 'GREEN-API env vars not configured', scanned: 0, alerts: 0 };
  }
  ...
  let chatId = null;
  try {
    const chatsRes = await fetch(`${BASE_URL}/getChats/${TOKEN}`);
    if (!chatsRes.ok) throw new Error(`getChats HTTP ${chatsRes.status}`);
    const chats = await chatsRes.json();
    const target = (Array.isArray(chats) ? chats : []).find(
      (c) => c.name && c.name.includes('משרד עורכי דין')
    );
    if (!target) {
      return { error: 'Group "משרד עורכי דין - משימות" not found', scanned: 0, alerts: 0 };
    }
    chatId = target.id;
  } catch (err) {
  ...
  let messages = [];
  try {
    const histRes = await fetch(`${BASE_URL}/getChatHistory/${TOKEN}`, {
      method: 'POST',
