...
const HARDCODED_CONFIG = {
  baseUrl: 'https://7107.api.greenapi.com/waInstance7107631283',
  token: '574296b4b6384a75a1136693c0a162e31316e872911349d082',
  groupName: 'משרד עורכי דין',
};

async function loadGreenApiConfig() {
...
export async function runWhatsappScan() {
  ...
  for (const msg of transfers) {
    ...
    // Check if already stored (table may not exist yet)
    const { data: existing, error: existErr } = await sb
      .from('whatsapp_alerts')
      .select('id')
      .eq('message_id', messageId)
      .limit(1);
    if (existErr) break; // whatsapp_alerts table not yet created
