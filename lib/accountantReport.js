...
const SECTION_HE = { office: 'עלויות משרדיות', personal: 'אישי / נכסים', ai: 'בינה מלאכותית וכלים' };
 ...
 * @returns { ok, sent, pending_contact?, totals, error? }
 */
export async function sendAccountantReport(sb, orgId, year, month) {
  ...
  const [
    ...
    sb.from('expense_documents')
      .select('expense_item, expense_section, file_name, file_url, amount, vendor, doc_date, status, payer')
  ...
  const bankTotal = 0; // bank transaction sync not yet active
  const bankVat = 0;
  const incomeTotal = (invoices || []).reduce((s, i) => s + Number(i.amount || 0), 0);
  const docsTotal = (expenseDocs || []).reduce((s, d) => s + Number(d.amount || 0), 0);
  ...
  if (!org?.accountant_email) {
    return {
      ...
      error: 'לא הוגדרו פרטי רו״ח. הדוח מוכן עקרונית אך ממתין להוספת פרטי רו״ח במערכת לפני שליחה.',
      month: monthName,
      totals: { officeTotal, bankTotal, bankVat, incomeTotal, docsTotal, expenseDocs: (expenseDocs || []).length },
    };
  }
  ...
  const summary = [
    ...
    ['  מזה — בינה מלאכותית וכלים', fmtIL(bySection.ai || 0)],
  ...
  const rows5 = [['סעיף', 'קטגוריה', 'ספק/תיאור', 'תאריך', 'סכום', 'קובץ', 'קישור', 'סטטוס']];
  (expenseDocs || []).forEach(d => rows5.push([
    d.expense_item || '', SECTION_HE[d.expense_section] || d.expense_section || '',
    d.vendor || '', d.doc_date || '', fmtIL(d.amount), d.file_name || '', d.file_url || '', d.status || 'pending',
  ]));
  rows5.push([]);
  rows5.push(['', '', '', 'סה"כ מתועד', fmtIL(docsTotal), '', '', '']);
  const ws5 = XLSX.utils.aoa_to_sheet(rows5);
  ws5['!cols'] = [{ wch: 28 }, { wch: 16 }, { wch: 28 }, { wch: 12 }, { wch: 12 }, { wch: 32 }, { wch: 70 }, { wch: 10 }];
  ...
  const html = `
    <div dir="rtl" style="font-family: Arial, sans-serif; line-height: 1.7;">
      ...
      <table style="border-collapse: collapse;" border="1" cellpadding="6">
        ...
        <tr><td>בינה מלאכותית וכלים</td><td>₪${fmtIL(bySection.ai || 0).toLocaleString('he-IL')}</td></tr>
  ...
  return {
    ...
    month: monthName,
    totals: { officeTotal, bankTotal, bankVat, incomeTotal, docsTotal, expenseDocs: (expenseDocs || []).length },
    error: sent ? null : 'שגיאה בשליחת המייל',
  };
}
