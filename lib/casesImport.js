...
async function syncCases(sb, orgId, profileByName, rows, sheetType) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    // Skip header-repeat rows (both name and attorney are column header texts)
    if (clientName === 'שם התיק/לקוח' || attorney === 'עו"ד מטפל') continue;
    // "תיקים אחרים": the sheet ends with a secondary plots-by-location table
    // whose "מס' תיק" cell holds a person name rather than a case number
    if (sheetType === 'other' && fields.caseNumber && !/\d/.test(fields.caseNumber)
        && !fields.attorney && !fields.idNumber) continue;
    // "תיקים אחרים": skip stray label rows that carry a name only and no case
    // data (the sheet has a small lookup list of place names at the bottom)
    if (sheetType === 'other'
        && !fields.caseNumber && !fields.attorney && !fields.idNumber
        && !fields.notes && !fields.feeRaw && !fields.collectedRaw && !fields.balanceRaw
        && !fields.paymentStatus && !fields.otherLawyer && !fields.referral
        && !fields.openDateRaw && !fields.targetDateRaw
        && (!fields.documents || fields.documents.length === 0)) continue;
