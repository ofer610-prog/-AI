...
import { decide, cardInContext } from '@/lib/scanEngine';
 ...
 */
export async function scanOrg(sb, org, daysBack = 120, opts = {}) {
  // opts.refreshToken — override which mailbox to scan (defaults to primary)
  // opts.idPrefix     — prefix stored gmail_message_id so a second mailbox's
  //                     message IDs can never collide with the primary's
  const refreshToken = opts.refreshToken || org.gmail_refresh_token;
  const idPrefix = opts.idPrefix || '';
  if (!refreshToken) return { skipped: 'no_gmail_token' };

  // כרטיסים — קבועים (1626, 9434) + מ-org + env
  ...
  for (const msg of found.values()) {
    try {
      const storedId = idPrefix + msg.id;
      if (await duplicateExists(sb, org.id, storedId)) { duplicates++; continue; }
      ...
      // וודא שה-4 ספרות מופיעות בהקשר כרטיס אשראי (לא מספר כלשהו אחר)
      if (card && !cardInContext([subject, bodyText].join(' '), card)) { skipped++; continue; }

      const d = decide({ subject, fromEmail: from, fromName: from, body: bodyText, hasAttachment: attach });
      // סריקה לפי כרטיס בלבד: חייב להיות סכום — בלי סכום → skip (לא review)
      if (d.action === 'skip' || !d.amount) { skipped++; continue; }
      ...
      // auto_import → expense_documents (Drive save is best-effort — לא חוסם שמירה)
      // שמירה ל-Drive אופציונלית: אם היא נכשלת (הרשאה/תיקייה/מכסה) עדיין שומרים
      // את הרשומה ל-DB עם קישור למייל, במקום לזרוק את כל המייל ל-failed.
      let saved = { url: null, fileName: null, source: null };
      try {
        saved = await saveGmailReceiptToDrive({
          org, gmailId: msg.id, mailboxRefreshToken: refreshToken,
          row: { subject, description: subject, amount, vendor, card_last4: card },
          docDate, year, month, topic: item || vendor, vendor,
        });
      } catch (driveErr) {
        console.warn('EXPENSE_GMAIL_SCAN drive save failed (non-fatal)', msg.id, driveErr.message);
      }
      await sb.from('expense_documents').insert({
        organization_id: org.id,
        amount,
        ...
        doc_date: docDate,
        month: docDate.slice(0, 7),
        // ספק מוכר בוודאות גבוהה (אגרות טאבו וכו') → אישור אוטומטי.
        // המשתמש עדיין יכול לערוך/למחוק. סיווג נדרש רק כשלא בטוח (נתיב review).
        status: 'approved',
...
/**
 * Scan every Gmail mailbox connected to an org (primary + optional second
 * "invoices" mailbox), merging the per-mailbox stats. The second mailbox's
 * message IDs are stored with a `g2_` prefix so dedup never collides across
 * the two inboxes.
 * @param {object} org — must include gmail_refresh_token and/or gmail2_refresh_token
 */
export async function scanOrgAll(sb, org, daysBack = 120) {
  const runs = [];
  if (org.gmail_refresh_token) {
    runs.push(await scanOrg(sb, org, daysBack)); // primary — no prefix (back-compat)
  }
  if (org.gmail2_refresh_token) {
    runs.push(await scanOrg(sb, org, daysBack, { refreshToken: org.gmail2_refresh_token, idPrefix: 'g2_' }));
  }
  if (!runs.length) return { skipped: 'no_gmail_token', accounts: 0 };

  const merged = { queries: 0, found: 0, imported: 0, pending_review: 0, duplicates: 0, skipped: 0, failed: 0, accounts: runs.length };
  for (const r of runs) {
    for (const k of ['queries', 'found', 'imported', 'pending_review', 'duplicates', 'skipped', 'failed']) {
