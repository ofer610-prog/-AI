...
/**
 * verifyGmailToken — בודק אם ה-refresh token עדיין תקף (לא invalid_grant).
 * מחזיר { ok: true } אם ניתן לקבל access token, אחרת { ok: false, reason }.
 * משמש את מסך הסטטוס כדי להציג "מחובר" רק כשהחיבור באמת עובד.
 */
export async function verifyGmailToken(refreshToken) {
  if (!refreshToken) return { ok: false, reason: 'no_token' };
  try {
    const oauth2Client = getOAuthClient();
    oauth2Client.setCredentials({ refresh_token: refreshToken });
    const res = await oauth2Client.getAccessToken();
    return { ok: !!res?.token, reason: res?.token ? null : 'no_access_token' };
  } catch (e) {
    const msg = String(e?.message || e);
    const dead = /invalid_grant|unauthorized|invalid_token/i.test(msg);
    return { ok: false, reason: dead ? 'invalid_grant' : msg };
  }
}
