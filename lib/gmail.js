...
// SECURITY: Use narrowest possible scopes per the article's recommendation.
// gmail.readonly = read messages (classification, sync)
// gmail.labels   = add/remove labels only (mark as processed) — no send/delete/modify
...
export async function classifyEmail(emailDetails) {
  ...
  if (!['income', 'expense', 'neutral'].includes(parsed.direction)) parsed.direction = 'neutral';
  if (!['high', 'medium', 'low'].includes(parsed.confidence)) parsed.confidence = 'low';
  if (parsed.amount === undefined) parsed.amount = null;
  if (parsed.date === undefined) parsed.date = null;
