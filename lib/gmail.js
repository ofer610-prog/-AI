...
import { buildSafePrompt, sanitizeForPrompt, parseAIJsonResponse } from '@/lib/security';
...
export async function classifyEmail(emailDetails) {
  ...
  // SECURITY: Email content is wrapped in safe delimiters to prevent prompt injection.
  // The model is instructed to ignore any instructions found inside the email body.
  const externalContent = [
    ...
    ``,
    `BODY:`,
    sanitizeForPrompt(emailDetails.body),
  ].join('\n');
  ...
  if (!parsed) {
    ...
    return { classification: 'other', is_relevant: false, confidence: 'low' };
  }

  // Validate that classification is one of the allowed values
  const VALID_CLASSIFICATIONS = [
    'bank-notification','bit-payment','invoice','transfer-confirmation',
    'whatsapp-export','client-payment','other',
  ];
  if (!VALID_CLASSIFICATIONS.includes(parsed.classification)) {
    parsed.classification = 'other';
  }

  // Validate amount is a positive number, not something injected
  if (parsed.amount !== null && (typeof parsed.amount !== 'number' || parsed.amount < 0 || parsed.amount > 10_000_000)) {
    parsed.amount = null;
  }
