...
export async function scanOutlookOrg(sb, org, days = 7) {
  ...
  const stats = { found: messages.length, pending_review: 0, duplicates: 0, skipped: 0 };
  ...
  for (const msg of messages) {
    ...
    if (insertErr) {
      console.error('Outlook gmail_processed insert error:', insertErr.message);
      stats.skipped++;
      continue;
    }
