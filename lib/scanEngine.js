...
// מחזירה true רק אם 4 הספרות מופיעות בפורמט כרטיס אמיתי:
//   *1626 | xxxx-1626 | כרטיס...1626 | ספרות אחרונות: 1626 | חיוב...1626
// כל מספר "חופשי" (מודעה, ערוץ, מחיר דירה) → false → מסוננ

const CARD_WORD = /(?:כרטיס|אשראי|חיוב\s*כרטיס|דביט|visa|mastercard|isracard|ישראכרט|כרטיסי|ספרות אחרונות|last\s*4|ending\s*in|debit\s*card|credit\s*card)/i;

export function cardInContext(text, card) {
  const t = String(text || '');
  const c = String(card);

  // פורמט 1: ****1626 / xxxx-1626 / *1626
  if (new RegExp(`[*xX]{2,}[-\\s]?${c}`).test(t)) return true;

  // פורמט 2: מילת כרטיס במרחק 70 תווים מהספרות
  let pos = t.indexOf(c);
  while (pos !== -1) {
    const window = t.slice(Math.max(0, pos - 70), pos + c.length + 70);
    if (CARD_WORD.test(window)) return true;
