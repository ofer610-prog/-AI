...
export const SKIP_DOMAINS = [
  ...
  'grow.security',
  // נדל"ן / פרסום (לא חשבוניות)
  'mail.yad2.co.il', 'yad2.co.il', 'madlan.co.il',
  // שיווק ספקים (לא חשבוניות — תת-דומיין שיווקי)
  'emailcamp.cellcom', 'marketing.cellcom', 'email.partner', 'campaign.hot',
  'noreply@news.', 'promo.', 'offers.',
];
...
export const SKIP_SUBJECT_KEYWORDS = [
  ...
  'פרסומת', 'פרסומות', 'מסר פרסומי', 'מסרי פרסום', 'דיוור פרסומי',
  'הטבות משתלמות', 'הטבות בלעדיות', 'מבצע בלעדי',
  'פסטיבל', 'טוסטר', 'רכב חדש', 'קרוז', 'נסיעה מאורגנת',
  // בידור / תוכן (לא תשלומים)
  'סדרות וסרטים', 'לצפות בסדרה', 'צפה עכשיו', 'לצפות עכשיו',
  'לא תאמינו', 'כדאי לדעת', 'חדשות מרגשות',
  // משלוחים ללא תשלום
...
// ── 4. בדיקת הקשר כרטיס אשראי ───────────────────────────────────────────────
// מוודאת שה-4 ספרות מופיעות בהקשר של כרטיס אשראי/חיוב — לא מספר כלשהו אחר.
// משמשת את הסורקים לפני decide() כדי לסנן מיילים לא-רלוונטיים.
const CARD_CONTEXT_RE = /(?:כרטיס|אשראי|חיוב|דביט|visa|mastercard|cal|max|isracard|ישראכרט|כרטיסי)\s*(?:\*+\d*|x+\d*|\d{0,4}[-\s])?\s*\d{0,4}[-\s*]*(\d{4})|(?:\*{2,}|x{2,})[-\s]?(\d{4})|(?:ספרות אחרונות|last.?4|last.?four|ending.?(?:in|with)?)\D*(\d{4})/gi;

/**
 * cardInContext(text, card)
 * מחזיר true אם 4 הספרות של הכרטיס מופיעות בטקסט בהקשר של כרטיס אשראי.
 * אם הטקסט קצר (נושא בלבד) — מחזיר true כדי לא לסנן יתר על המידה.
 */
export function cardInContext(text, card) {
  const t = String(text || '');
  if (t.length < 80) return true; // נושא בלבד — אל תסנן
  const cardStr = String(card);
  // בדוק אם ה-4 ספרות מופיעות ב-window של 60 תווים מהקשר כרטיס
  const re = new RegExp(CARD_CONTEXT_RE.source, 'gi');
  let m;
  while ((m = re.exec(t)) !== null) {
    const found = (m[1] || m[2] || m[3] || '').trim();
    if (found === cardStr) return true;
  }
  // גיבוי: ספרות מסוימות בסמוך למילת "חיוב" / "כרטיס"
  const nearby = /(?:כרטיס|חיוב|אשראי|credit|debit|card)[^0-9]{0,40}/gi;
  let n;
  while ((n = nearby.exec(t)) !== null) {
    const segment = t.slice(n.index, n.index + n[0].length + 20);
    if (segment.includes(cardStr)) return true;
  }
  return false;
}
