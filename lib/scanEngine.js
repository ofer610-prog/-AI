...
export function decide(email) {
  ...
  // ── שלב ב: זיהוי ──
  const sup = detectSupplier(haystackLow);
  ...
  // ── שלב ג: החלטה ──
  // ייבוא אוטומטי: ספק מוכר + סכום תקין (חזק) + לא חריג + לא מסומן noAutoImport
  if (sup && !sup.noAutoImport && amount && amount <= AUTO_IMPORT_MAX && strong) {
    return { action: 'auto_import', reason: `ספק מוכר (${sup.label}) + סכום מאומת`, confidence: 'high', ...base };
  }
...
function skip(reason) {
  return { action: 'skip', reason, vendor: null, supplierId: null, amount: null,
