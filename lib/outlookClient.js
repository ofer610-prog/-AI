...
/**
 * Search for emails matching a filter string using Graph API ($filter / OData).
...
export async function searchOutlookMessages(accessToken, filter, top = 50) {
  ...
  for (let page = 0; page < 3; page++) {
    ...
    const res = await fetch(fullUrl.toString(), { headers: { Authorization: `Bearer ${accessToken}` } });
    if (!res.ok) break;
...
/**
 * Full-text search using KQL ($search parameter) — searches subject + body.
 * Used for card-number-based scanning (same approach as Gmail card queries).
 * Note: $search and $filter cannot be combined; filter by date in caller code.
 *
 * @param {string} accessToken
 * @param {string} keyword   — the card last-4 to search for (e.g. "1626")
 * @param {number} maxPages  — pages of 50 messages to fetch (default 4 = 200 msgs)
 */
export async function searchOutlookByKeyword(accessToken, keyword, maxPages = 4) {
  const messages = [];
  const params = new URLSearchParams({
    $search: `"${keyword}"`,
    $top: '50',
