...
// "Today" as YYYY-MM-DD in Israel time — crons run on UTC servers, so plain
// toISOString() can be a day behind/ahead around midnight Israel time.
export const israelToday = (offsetDays = 0) => {
  const d = new Date(Date.now() + offsetDays * 86400000);
  return d.toLocaleDateString('en-CA', { timeZone: 'Asia/Jerusalem' }); // en-CA → YYYY-MM-DD
};

// Start/end of the current Israel-local day, as Date objects (for timestamp queries)
export const israelDayRange = () => {
  const dateStr = israelToday();
  // Determine current Israel UTC offset (+02:00 or +03:00 in DST)
  const offsetMin = (new Date(new Date().toLocaleString('en-US', { timeZone: 'Asia/Jerusalem' }))
    - new Date(new Date().toLocaleString('en-US', { timeZone: 'UTC' }))) / 60000;
  const sign = offsetMin >= 0 ? '+' : '-';
  const abs  = Math.abs(offsetMin);
  const tz   = `${sign}${String(Math.floor(abs / 60)).padStart(2, '0')}:${String(abs % 60).padStart(2, '0')}`;
  return {
    start: new Date(`${dateStr}T00:00:00${tz}`),
    end:   new Date(`${dateStr}T23:59:59.999${tz}`),
  };
};

// Parse a date that may be DD/MM/YYYY, D.M.YY, YYYY-MM-DD, or a Date object,
// plus an optional time string, into a Postgres-safe timestamp string.
// Source spreadsheets are usually DD/MM/YYYY, but some feeds send M/D/YYYY —
// when the parsed month is > 12 the parts are swapped, so we swap them back.
// (Previously this produced invalid literals like "2026-30-08" that Postgres
// rejected with "date/time field value out of range".)
export function dmyToISO(dateVal, timeStr) {
  ...
  if (dateVal instanceof Date) {
    ...
    if (dmy) {
      let [, d, m] = dmy;
      const y = dmy[3];
      if (Number(m) > 12 && Number(d) <= 12) { const tmp = d; d = m; m = tmp; }
