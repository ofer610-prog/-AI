...
export async function saveGmailReceiptToDrive({ org, gmailId, mailboxRefreshToken, row = {}, docDate, year, month, topic, vendor }) {
  // The mailbox token reads the email (may be the second Gmail account);
  // the Drive token owns the shared expenses folder (always the primary account).
  const mailboxToken = mailboxRefreshToken || org?.gmail_refresh_token;
  const driveToken   = org?.gmail_refresh_token || mailboxRefreshToken;
  if (!mailboxToken || !driveToken) return { url: null, fileName: null, note: 'אין refresh token' };
  if (!gmailId) return { url: null, fileName: null, note: 'אין מזהה מייל' };

  const gmail = getGmailClient(mailboxToken);
