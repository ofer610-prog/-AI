...
export async function GET(request) {
  ...
  console.log('[google-oauth] callback-start', JSON.stringify({ hasCode: !!code, error: error || null, hasState: !!stateParam, validState: !!stateData }));
  ...
  if (!orgId) {
    try {
      ...
      console.log('[google-oauth] session-fallback', JSON.stringify({ hasUser: !!user }));
      if (!user) return back(request, { gmail_error: 'session_lost' });
      const { data: profile, error: profileError } = await supabase
        .from('profiles').select('organization_id').eq('id', user.id).single();
      console.log('[google-oauth] profile', JSON.stringify({ hasProfile: !!profile, profileError: profileError?.message || null }));
      orgId = profile?.organization_id;
    } catch (e) {
      console.error('[google-oauth] session-fallback-error', e.message);
    }
  }
  ...
  try {
    ...
    console.log('[google-oauth] tokens', JSON.stringify({ hasAccess: !!tokens.access_token, hasRefresh: !!tokens.refresh_token, scope: tokens.scope || null }));
    ...
    const { data: existing, error: existingError } = await sb
      ...
      .eq('id', orgId)
      .single();
    console.log('[google-oauth] existing', JSON.stringify({ existingError: existingError?.message || null, hadRefresh: !!existing?.gmail_refresh_token }));

    if (!tokens.refresh_token && !existing?.gmail_refresh_token) {
      console.warn('[google-oauth] missing-refresh-token');
    ...
    console.log('[google-oauth] update-result', JSON.stringify({ updateError: updateError?.message || null, savedConnected: !!updated?.gmail_connected, savedEmail: updated?.gmail_email || null, savedRefresh: !!updated?.gmail_refresh_token }));
